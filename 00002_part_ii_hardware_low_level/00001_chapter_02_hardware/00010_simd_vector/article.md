...
Когда мы умножаем две матрицы $1000 \times 1000$, мы выполняем миллиард операций умножения и сложения. Если процессор обрабатывает по одному числу за такт, это займёт секунду при частоте 1 ГГц. Но реальные процессоры делают это в десятки раз быстрее — благодаря SIMD.
...
Одна инструкция, 8 результатов. Ускорение $8\times$ при той же частоте. Это прямое следствие закона Флинна: векторный процессор относится к классу SIMD в таксономии вычислительных архитектур.

### 1.2 Таксономия SIMD инструкций

| Архитектура | Ширина вектора | Название |
|-------------|----------------|---------- |
| x86: MMX | 64 бит ($8 \times 8$ или $4 \times 16$ или $2 \times 32$) | 1997 |
| x86: SSE/SSE2 | 128 бит ($4 \times$ float или $2 \times$ double) | 1999/2001 |
| x86: AVX/AVX2 | 256 бит ($8 \times$ float или $4 \times$ double) | 2011/2013 |
| x86: AVX-512 | 512 бит ($16 \times$ float или $8 \times$ double) | 2016 |
...
Практические результаты на Core i7 ($8 \times$ float за итерацию при AVX):
- Скалярная: ~60 мс
- AVX: ~12 мс (ускорение ${\sim}5\times$, не $8\times$ из-за накладных расходов памяти)
