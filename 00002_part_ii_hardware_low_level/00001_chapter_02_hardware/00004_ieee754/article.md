...
Для нормализованных чисел (экспонента $\neq 0$ и $\neq 255$) мантисса всегда начинается с 1. Эта единица не хранится — она подразумевается. Это позволяет использовать один дополнительный бит точности.
...
NaN — результат неопределённых операций: $0/0$, $\sqrt{-1}$, $\infty - \infty$, $\infty \times 0$.
...
**Машинный эпсилон ($\varepsilon$)** — наименьшее число, такое что `1.0 + ε ≠ 1.0`.
