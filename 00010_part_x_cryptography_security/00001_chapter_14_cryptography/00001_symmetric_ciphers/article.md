...
AES работает с 16-байтным блоком, представляя его как матрицу 4×4 байта, называемую **состоянием** (state):
...
Каждый байт заменяется по таблице замены **S-box** (Substitution Box). S-box AES — это не произвольная таблица, а математически обоснованная конструкция: для каждого байта вычисляется мультипликативный обратный элемент в поле GF(2⁸), затем применяется аффинное преобразование.
...
#### MixColumns — умножение в поле GF(2⁸)

Каждый столбец матрицы умножается на фиксированную матрицу в поле Галуа GF(2⁸). Это максимизирует диффузию: каждый входной байт влияет на все 4 байта столбца.
...
GCM = CTR + GHASH (полиномиальный хеш в поле GF(2¹²⁸)).
...
- **A**ddition (сложение по модулю 2³²)
...
ChaCha20 работает с **состоянием 4×4 матрицы 32-битных слов** (512 бит = 64 байта):
