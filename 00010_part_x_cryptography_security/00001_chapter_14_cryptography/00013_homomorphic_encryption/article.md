...
- Потеря точности $\sim 10^{-10}$ — приемлемо для ML
...
|-----------------|----------------|-----------------|-----------|
| Сложение float   | 1 нс           | ~1 мкс          | $1000\times$     |
| Умножение float  | 1 нс           | ~10 мкс         | $10000\times$    |
| Bootstrapping   | N/A            | ~1 сек          | —         |
| Нейронная сеть (simple) | ~10 мс | ~10 мин        | $60000\times$    |

На GPU и специализированных ускорителях (Intel HEXL, специализированные FPGA/ASIC):
- $10\text{--}100\times$ ускорение по сравнению с CPU
- Это всё равно в $100\text{--}1000\times$ медленнее обычных вычислений
