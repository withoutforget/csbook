...
Алгоритм Шора позволяет квантовому компьютеру факторизовать число N за время $O((\log N)^3)$, что полиномиально. Это уничтожает RSA.
...
- SHA-256: коллизии за $O(2^{128})$ вместо $O(2^{128})$ — безопасно
- SHA-512: безопасно
...
Решётка — это дискретное подпространство $\mathbb{R}^n$: множество всех целочисленных линейных комбинаций базисных векторов.
