...
- **Constant propagation**: если $x_1 = 1$, заменить все $x_1$ на 1
- **Dead code elimination**: если $x_3$ никогда не используется — удалить присваивание
...
**Суперпозиция**: кубит $|\psi\rangle = \alpha|0\rangle + \beta|1\rangle$, где $|\alpha|^2 + |\beta|^2 = 1$.
...
- **Шор**: факторизация за $O((\log N)^3)$ vs классический $O(\exp((\log N)^{1/3}))$
- **Гровер**: поиск $O(\sqrt{N})$ vs $O(N)$
...
Дискретное Преобразование Фурье (DFT) — преобразование сигнала из временного домена в частотный. Вычислительная сложность: $O(N^2)$.
