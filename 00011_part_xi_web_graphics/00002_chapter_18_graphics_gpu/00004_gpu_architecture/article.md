...
Почему нейросети обучаются на GPU, а не на CPU? Почему умножение матриц $4096 \times 4096$ на GPU занимает миллисекунды, а на CPU — секунды? Ответ в фундаментальном различии архитектур: CPU оптимизирован для последовательных вычислений с быстрым откликом, GPU — для максимальной параллельной пропускной способности.
...
| Матрица $4096 \times 4096$ FP32 | ~100ms | ~1ms | Идеальная параллельность |
