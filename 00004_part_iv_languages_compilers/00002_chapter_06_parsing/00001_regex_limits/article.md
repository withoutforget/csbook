...
- Алфавитом $\Sigma$
- Функцией переходов $\delta: Q \times \Sigma \to Q$
- Начальным состоянием $q_0$
...
- $\varepsilon$-переходы (без чтения символа)
...
- Альтернатива `A|B` → добавить начальное состояние с $\varepsilon$-переходами в оба
- Повторение `A*` → добавить $\varepsilon$-петлю
