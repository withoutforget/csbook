...
**Атака HashDoS:** Злоумышленник, зная алгоритм хеширования веб-сервера, отправляет HTTP запросы с параметрами, хеш-коллизии которых заполняют одну корзину хеш-таблицы → O(n²) вместо O(n) → DoS.
