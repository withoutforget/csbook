...
Пусть `p` — доля программы, которая может выполняться параллельно ($0 \leq p \leq 1$). Тогда максимальное ускорение при использовании `n` процессоров:

```
S(n) = 1 / ((1 - p) + p/n)
```

При $n \to \infty$: $S(\infty) = 1 / (1 - p)$
