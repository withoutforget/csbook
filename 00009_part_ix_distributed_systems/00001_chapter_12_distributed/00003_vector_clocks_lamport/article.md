...
Каждый из N процессов хранит вектор из N счётчиков: $V = [v_1, v_2, \ldots, v_n]$, где $v_i$ — количество событий, о которых этот процесс «знает» на процессе i.
...
HLC используется в CockroachDB, TiDB, и ряде других NewSQL-систем. Метка HLC монотонна, отличается от реального времени не более чем на параметр $\varepsilon$ (обычно настраиваемый порог), и сохраняет свойства happens-before.
...
Где $\varepsilon$ обычно < 7 мкс. Для записи Spanner ждёт, пока commit timestamp гарантированно окажется в прошлом (commit wait: ждать $2\varepsilon$ до подтверждения). Это позволяет реализовать внешнюю согласованность (external consistency): если транзакция B началась после завершения транзакции A, то timestamp(A) < timestamp(B).
