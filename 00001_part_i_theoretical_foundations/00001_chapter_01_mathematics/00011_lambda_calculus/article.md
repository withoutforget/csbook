...
Примеры λ-термов:
- `x` — переменная x
- `λx.x` — тождественная функция (identity)
- `λx.λy.x` — функция двух аргументов, возвращающая первый
- `(λx.x) y` — применение identity к y
...
```
λx.f x  =η  f    (если x не входит свободно в f)
```
...
```
0 = λf.λx.x           — применить f ноль раз
1 = λf.λx.f x          — применить f один раз
2 = λf.λx.f(f x)       — применить f дважды
n = λf.λx.f^n x        — применить f n раз
```
...
Y f = f(Y f)    — Y f является неподвижной точкой f
```
...
Каждое выражение имеет тип. Базовые типы: `int`, `bool`. Составные: `A → B` (функция из A в B).

```
Γ ⊢ x : A           — переменная x имеет тип A в контексте Γ
Γ, x:A ⊢ M : B      
-----------------    — абстракция
Γ ⊢ λx.M : A → B    
...
|---|---|---|
| Терм M | Доказательство φ | Программа типа A |
| Тип A → B | Импликация A ⟹ B | Функциональный тип |
| Тип A × B | Конъюнкция A ∧ B | Тип пары |
| Тип A + B | Дизъюнкция A ∨ B | Тип суммы (Either) |
| Тип ⊥ | Ложь | Необитаемый тип (Never) |
