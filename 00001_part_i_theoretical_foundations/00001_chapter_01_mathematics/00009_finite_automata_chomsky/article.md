...
NFA позволяет несколько переходов по одному символу и ε-переходы (переходы без чтения символа). Мощность распознавания — та же, что у DFA.
...
Регулярные выражения — алгебраическая нотация для регулярных языков. Для алфавита Σ:

- ∅: пустой язык
- ε: язык, содержащий только пустую строку
- a ∈ Σ: язык {a}
- R₁ | R₂: объединение языков
- R₁R₂: конкатенация
...
**Лемма о накачке (для регулярных языков)**: если L — регулярный язык, то существует p (длина накачки), такое что любая строка w ∈ L с |w| ≥ p может быть представлена как w = xyz, где:
1. |y| > 0
2. |xy| ≤ p
3. для всех k ≥ 0: xyᵏz ∈ L

**Следствие**: язык {aⁿbⁿ | n ≥ 0} **нерегулярен**: он требует «запоминать» количество a, а DFA имеет лишь конечную память.
...
Включение строгое: Уровень 3 ⊂ Уровень 2 ⊂ Уровень 1 ⊂ Уровень 0.
