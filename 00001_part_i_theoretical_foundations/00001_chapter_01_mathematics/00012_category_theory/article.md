...
- **Объектов** (objects): коллекции, обозначаемые ob(C)
- **Морфизмов** (morphisms/arrows): для каждой пары A, B ∈ ob(C) — коллекция hom(A, B) морфизмов из A в B
- **Операции композиции**: ∘: hom(B,C) × hom(A,B) → hom(A,C)
- **Тождественных морфизмов**: для каждого A ∈ ob(C) — id_A ∈ hom(A,A)

Со следующими аксиомами:
- **Ассоциативность**: (h ∘ g) ∘ f = h ∘ (g ∘ f)
- **Единица**: id_B ∘ f = f = f ∘ id_A (для f: A → B)
...
Функтор F: C → D — отображение между категориями, сохраняющее структуру:
- Объект A ∈ C отображается в F(A) ∈ D
- Морфизм f: A → B отображается в F(f): F(A) → F(B)
...
Натуральное преобразование η: F → G — семейство морфизмов {η_A: F(A) → G(A)}, «коммутирующих с функторами»: для любого f: A → B выполняется η_B ∘ F(f) = G(f) ∘ η_A.
...
Моноид (M, ⊕, e) — множество M с операцией ⊕ и нейтральным элементом e:
- **Замкнутость**: a ⊕ b ∈ M
- **Ассоциативность**: (a ⊕ b) ⊕ c = a ⊕ (b ⊕ c)
- **Нейтральный элемент**: e ⊕ a = a = a ⊕ e
...
Математически: монада в категории C — это тройка (T, η, μ), где:
- T: C → C — функтор
- η: Id → T — единица (unit/return)
- μ: T² → T — умножение (join)

Удовлетворяющие законам:
- μ ∘ Tη = id = μ ∘ ηT (левая и правая единицы)
- μ ∘ Tμ = μ ∘ μT (ассоциативность)
