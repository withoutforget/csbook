...
Поиск «похожих» векторов — задача Approximate Nearest Neighbor Search (ANN). Точный поиск ближайшего соседа в 1536-мерном пространстве среди 100 миллионов векторов требует O(N×D) операций — неприемлемо медленно. Векторные СУБД решают эту задачу с помощью специализированных индексов: HNSW, IVF, PQ и их комбинаций.
...
| Мультимодальные | CLIP, ImageBind | 512+ | Текст ↔ изображение |
...
- Брутфорс: O(N×D) — для 1M векторов × 1536 dim = 1.5 млрд операций
