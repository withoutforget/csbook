...
Система из двух узлов A и B, хранящих переменную v с начальным значением v₀.

1. Клиент записывает v₁ в узел A
2. Между A и B происходит разбиение сети (A не может отправить v₁ в B)
3. Другой клиент читает с узла B

Возможны два сценария:
- **Узел B отвечает v₀**: согласованность нарушена (клиент прочитал устаревшее значение), но доступность сохранена
- **Узел B отвечает ошибкой**: согласованность сохранена (нет ответа = нет ошибочного ответа), но доступность нарушена

Третьего варианта нет: B не может знать актуальное v₁ без связи с A.
...
Google Spanner — интересное исключение: система утверждает, что обеспечивает external consistency (≈ strict serializability) при масштабировании на тысячи узлов по всему миру.
