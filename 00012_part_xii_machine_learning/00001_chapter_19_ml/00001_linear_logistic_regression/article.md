...
Нормальное уравнение: $O(n^3)$ из-за обращения матрицы. При большом числе признаков (n > 10000) — слишком медленно. Тогда используют градиентный спуск.
...
Проблема: если признаки сильно коррелируют между собой, матрица $X^\top X$ почти вырожденная → нестабильные веса.
