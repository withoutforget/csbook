...
После отправки финального ACK клиент входит в TIME_WAIT на **2 × MSL** (Maximum Segment Lifetime, обычно 60 секунд → TIME_WAIT = 120 секунд).
...
При установке нового соединения cwnd начинается маленьким (обычно 10 × MSS на современном Linux):
...
2. **TIME_WAIT**: 2×MSL (120 секунд) после активного закрытия. Нормально — не «лишние» соединения. `SO_REUSEADDR` для серверов.
