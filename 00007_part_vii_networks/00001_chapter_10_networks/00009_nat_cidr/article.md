...
NAT — это костыль, решающий проблему адресного пространства ценой нарушения сквозной (end-to-end) модели интернета. IPv6 с 128-битными адресами ($2^{128}$ — астрономическое число) призван навсегда решить проблему без NAT.
...
IPv6 (RFC 2460) предоставляет $2^{128} \approx 3.4 \times 10^{38}$ адресов — по ~50 квадриллионов на каждый квадратный метр поверхности Земли.
...
3. **NAT/PAT**: один публичный IP → тысячи устройств. Таблица трансляции (IP:Port $\leftrightarrow$ IP:Port).

4. **Проблемы NAT**: нарушает end-to-end, сложность P2P. Решение: NAT Traversal (STUN/TURN/ICE).

5. **IPv6**: $2^{128}$ адресов — решение без NAT. SLAAC — автоконфигурация без DHCP.
