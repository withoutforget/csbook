...
Говорят, что f(n) = O(g(n)), если существуют константы c > 0 и n₀ ≥ 0, такие что для всех n ≥ n₀ выполняется:

```
f(n) ≤ c · g(n)
```

Иными словами, начиная с некоторого n₀, функция f не превышает g, умноженную на константу. Это **верхняя** асимптотическая граница. Когда мы говорим, что алгоритм имеет сложность O(n²), мы утверждаем, что он растёт не хуже, чем квадрат от n — хотя может быть и лучше.

Пример: 3n² + 5n + 7 = O(n²), поскольку при c = 4 и n₀ = 10 неравенство выполняется.

### Ω (большая омега) — нижняя граница

f(n) = Ω(g(n)), если существуют c > 0 и n₀ ≥ 0, такие что для всех n ≥ n₀:

```
f(n) ≥ c · g(n)
```

Это **нижняя** граница. Если алгоритм имеет сложность Ω(n log n), значит, он не может быть быстрее, чем n log n, начиная с некоторого n. Сортировка сравнением имеет нижнюю границу Ω(n log n) — это фундаментальный результат теории информации.

### Θ (тета) — точная граница

f(n) = Θ(g(n)), если f(n) = O(g(n)) и f(n) = Ω(g(n)) одновременно. Это означает, что алгоритм растёт **точно** как g(n) с точностью до константы. Например, merge sort — Θ(n log n) в любом случае.

### o (малое о) и ω (малая омега)

Малые нотации означают **строгие** границы без равенства:

- f(n) = o(g(n)): lim_{n→∞} f(n)/g(n) = 0 (f растёт строго медленнее g)
- f(n) = ω(g(n)): lim_{n→∞} f(n)/g(n) = ∞ (f растёт строго быстрее g)
...
Почему это работает? При n = 10⁶ разница между n и 100n составляет множитель 100, но разница между n и n² составляет 10⁶ раз. На масштабе это несравнимо важнее.

Правила упрощения:
1. O(c · f(n)) = O(f(n)) для любой константы c
...
Перебор всех перестановок. При n = 20 количество перестановок — 2,4 × 10¹⁸.
...
- Худший случай: O(n²) — опорный всегда оказывается минимумом или максимумом (отсортированный массив с pivot = первый элемент)
...
Для динамического массива: Φ = 2·size - capacity.
...
1. Если f(n) = O(n^(c_crit - ε)), то T(n) = Θ(n^c_crit)
2. Если f(n) = Θ(n^c_crit · log^k(n)), то T(n) = Θ(n^c_crit · log^(k+1)(n))
3. Если f(n) = Ω(n^(c_crit + ε)), то T(n) = Θ(f(n))
...
1. **Константы важны на практике**: алгоритм O(n log n) с большой константой может проигрывать O(n²) при малых n. Вот почему интросорт использует insertion sort для маленьких подмассивов.
...
3. **Средний случай часто важнее**: quicksort используется повсеместно, несмотря на теоретический O(n²) в худшем случае, потому что средний O(n log n) — прекрасен на практике.
