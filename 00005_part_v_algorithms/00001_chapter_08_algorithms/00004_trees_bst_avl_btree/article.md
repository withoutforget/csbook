...
AVL-дерево (Адельсон-Вельский и Ландис, 1962) — первое самобалансирующееся BST. Инвариант: **разница высот левого и правого поддеревьев любого узла $\leq 1$**.

Высота AVL-дерева: O(log n). Точнее: $h \leq 1.44 \cdot \log_2(n+2) - 0.328$.
...
Гарантия: высота $\leq 2 \cdot \log_2(n+1)$ — O(log n), но с большей константой чем AVL.
...
B-tree со степенью t=100: каждый узел содержит до 199 ключей. Высота $\leq \log_{100}(1\,000\,000) = 3$. Всего 3 операции чтения!
...
2. **Узлы = страницы:** Размер узла $\approx$ размер страницы диска
