...
|----------|-------|--------|
| Вставка слова длины m | O(m) | O(m * |Σ|) |
| Поиск слова длины m | O(m) | — |
| Все слова с префиксом | O(m + k) | — |
| Память | — | O(N * |Σ|) |

Где N — суммарная длина всех слов, |Σ| — размер алфавита (26 для латиницы).
...
Aho-Corasick (1975) — алгоритм для одновременного поиска нескольких паттернов в тексте за O(n + Σk + выходы).
...
| Trie | O(m) | O(m) | O(m+k) | O(N·|Σ|) | Автодополнение, словари |
| Radix Tree | O(m) | O(m) | O(m+k) | O(N) | IP routing, сжатый Trie |
| TST | O(m log|Σ|) | O(m log|Σ|) | O(m+k) | O(N) | Балансировка памяти/скорости |
| Suffix Array | — | O(m log n) | — | O(n) | Поиск подстрок |
| Suffix Tree | — | O(m) | — | O(n) | Сложные строковые задачи |
| Aho-Corasick | O(ΣP) build | O(n+k) | — | O(ΣP) | Multi-pattern search |
