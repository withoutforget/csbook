...
**Теорема:** Любая сортировка, основанная на сравнениях, требует $\Omega(n \log n)$ операций в худшем случае.

**Доказательство через дерево решений:** Алгоритм сортировки — это бинарное дерево, где каждый внутренний узел — сравнение, а листья — перестановки. Для n элементов есть n! перестановок, значит n! листьев. Высота дерева $\geq \log_2(n!) = \Theta(n \log n)$ по формуле Стирлинга.
...
|----------|------------|--------------|--------|-----------|-----------|
| Quicksort | O(n log n) | $O(n^2)$ | O(log n) | Нет | Универсальная |
